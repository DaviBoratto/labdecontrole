...
void F_ISR(){
  ...
  // Leitura
  leitura_pot = analogRead(potenciometro);            // Leitura da referência (Potenciômetro)
  leitura_tensao = analogRead(tensao_saida_gerador);  // Leitura da saída do sistema (Tensão Gerador)

  leitura_pot_volts = round(analogRead(potenciometro) / 204.6);
  leitura_tensao_volts = analogRead(tensao_saida_gerador) / 202.1739;

  leitura_pot = round(analogRead(potenciometro) / 204.6);
  leitura_tensao = analogRead(tensao_saida_gerador) / 202.1739;
...
void loop(){
  ...
  analogWrite(sinal_pwm_interrupcoes, 55);      // Sinal PWM qualquer de 7812.5Hz para servir de base para as interrupções 
